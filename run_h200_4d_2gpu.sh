#!/usr/bin/env bash
#SBATCH -J mojo-h200-4d-2gpu
#SBATCH -o /lustre/bio/mdick/CUDA/PROFILES/trip-mojo/h200_4d_2gpu_%j.out
#SBATCH -e /lustre/bio/mdick/CUDA/PROFILES/trip-mojo/h200_4d_2gpu_%j.err
...
GPU_POOL_CHUNK_PERCENT="${GPU_POOL_CHUNK_PERCENT:-28}"
GPU_POOL_SIZE_PERCENT="${GPU_POOL_SIZE_PERCENT:-80}"
JOB_ID="${SLURM_JOB_ID:-manual}"
OUTDIR="${PROFILES}/h200_4d_2gpu_${JOB_ID}"
WORK="${SLURM_TMPDIR:-/tmp}/trip-mojo-h200-4d-2gpu-${JOB_ID}"
...
echo "job=${JOB_ID} host=$(hostname) devices=2 control_threads=${CONTROL_THREADS} pack_threads=${PACK_THREADS} gpu_pool=${GPU_POOL_CHUNK_PERCENT}x${GPU_POOL_SIZE_PERCENT}% output=${OUTDIR}"
echo "trip_temp_commit=$(git -C "${TRIP}" rev-parse HEAD)"
echo "trip_mojo_commit=$(git -C "${REPO}" rev-parse HEAD)"
nvidia-smi

nvidia-smi \
  --query-gpu=timestamp,index,name,power.draw,utilization.gpu,memory.used,temperature.gpu \
...
apptainer exec --nv -B /lustre:/lustre -B "${WORK}:${WORK}" \
  "${CONTAINER}" env \
    ...
    MODULAR_DEVICE_CONTEXT_MEMORY_MANAGER_CHUNK_PERCENT="${GPU_POOL_CHUNK_PERCENT}" \
    MODULAR_DEVICE_CONTEXT_MEMORY_MANAGER_SIZE_PERCENT="${GPU_POOL_SIZE_PERCENT}" \
    TRIP_FDCB_MOJO=1 TRIP_FDCB_MOJO_DEVICES=2 \
...
grep -E 'accelerator devices=|FDCB Mojo physical matrix|FDCB Mojo pack|FDCB Mojo optimize|FDCB Mojo:|OptVoxelSetup:|OptCmd:' \
