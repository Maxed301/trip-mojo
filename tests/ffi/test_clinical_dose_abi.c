...
_Static_assert(sizeof(ClinicalDosePoint) == 40, "point ABI layout");
_Static_assert(offsetof(ClinicalDosePoint, particles) == 32,
               "point particle offset");
_Static_assert(sizeof(ClinicalDoseDDDEntry) == 40, "DDD entry ABI layout");
_Static_assert(sizeof(ClinicalDoseDDDTable) == 12, "DDD table ABI layout");
_Static_assert(sizeof(ClinicalDoseBioEntry) == 48, "bio entry ABI layout");
_Static_assert(sizeof(ClinicalDoseBioTable) == 16, "bio table ABI layout");
_Static_assert(sizeof(ClinicalDoseEnergy) == 64, "energy ABI layout");
_Static_assert(sizeof(ClinicalDoseField) == 32, "field ABI layout");
_Static_assert(sizeof(ClinicalDoseGrid) == 88, "grid ABI layout");
_Static_assert(sizeof(ClinicalDoseCTState) == 144, "CT state ABI layout");
_Static_assert(sizeof(ClinicalDoseState) == 8, "state ABI layout");
_Static_assert(sizeof(ClinicalDosePosition) == 24, "position ABI layout");
_Static_assert(sizeof(ClinicalDoseGridField) == 184,
               "grid field ABI layout");
_Static_assert(sizeof(ClinicalDoseOutput) == 48, "output ABI layout");
_Static_assert(sizeof(ClinicalDoseProblem) == 344,
               "problem view ABI layout");

int main(void) {
    const ClinicalDoseGrid grid = {
    ...
    const ClinicalDoseCTState ct_state = {grid, 0, 0, 2, 4, 0.0, 0.0, 0};
    ClinicalDoseGridField grid_field;
    ...
    const ClinicalDoseField field = {0, 1, 100.0f, 8832.0f, 7806.0f};
    ClinicalDoseEnergy energy = {
        0, 4, 0, 0, 1.0f, 0.0f, -10.0f, 10.0f, -10.0f, 10.0f
    };
    ClinicalDosePoint points[4] = {
        {0.5f, 0.5f, 0.0f, 100.0f, 10.0},
        {0.5f, 0.5f, 0.0f, 100.0f, 5.0},
        {0.5 + sqrt(0.75), 0.5 + sqrt(0.75), 0.0f, 1.0f, 1.0},
        {0.5f, 20.0f, 0.0f, 1.0f, 1.0e9}
    };
    const ClinicalDoseDDDTable ddd_table = {0, 2, 5};
    const ClinicalDoseDDDEntry ddd_entries[2] = {
        {0.0f, 2.0f, 1.0f, 0.0f, 1.0f},
        {1.0f, 2.0f, 1.0f, 0.0f, 1.0f}
    };
    const ClinicalDoseBioTable bio_table = {0, 2, 1.0f};
    ClinicalDoseBioEntry bio_entries[2] = {
        {0.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f},
        {1.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f}
    };
    ClinicalDoseProblem problem;
    ...
    assert(trip_compute_clinical_dose(&problem, &output, 0) == -2);
    problem.struct_size = 0;
    assert(trip_compute_clinical_dose(&problem, &output, 1) == -1);
    problem.struct_size = sizeof(problem);
    ...
    ClinicalDoseOutput accelerator_output;
    problem.voxel_voi = NULL;
    assert(trip_compute_clinical_dose_on_device(
    ...
    const ClinicalDoseDDDEntry extrapolated_ddd[2] = {
        {0.0, 0.0, 1.0, 0.0, 1.0},
        {1.0, 1.0, 1.0, 0.0, 1.0}
    };
    ...
    const int32_t accelerator_status =
        trip_compute_clinical_dose_on_device(&problem, &output, 1);
    ...
    const ClinicalDoseCTState ct_states[2] = {ct_state, ct_state};
    const ClinicalDoseState states[2] = {{0, 1}, {1, 1}};
    ClinicalDoseGridField state_grid_fields[2] = {grid_field, grid_field};
    state_grid_fields[1].field_index = 1;
    ClinicalDoseField state_fields[2] = {field, field};
    ClinicalDosePosition transformed[2] = {
